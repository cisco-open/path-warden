...
echo "${LGrn}Deploying Demo Customer-Data...${NC}"
kubectl apply -f "$(dirname $0)/deploy.yml"
kubectl apply -f "$(dirname $0)/opal-client.yml"

#pod=$(echo $(kubectl get pods -l "app.kubernetes.io/name=customer-data" -o custom-columns=:metadata.name | tr -d "\n"))
...
echo "${LGrn}Demo Customer-Data successfully launched!"
