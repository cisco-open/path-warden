...
def test():
    ...
    response = requests.get(url="http://inventory.default.svc.cluster.local:80/test", headers=request_headers)
    response_labels.append(response.headers.get("X-Response-Labels"))
    print("response_labels:", response_labels, type(response_labels))

    response = requests.get(url="http://shipping.default.svc.cluster.local:80/test", headers=request_headers)
    response_labels.append(response.headers.get("X-Response-Labels"))
    print("response_labels:", response_labels, type(response_labels))
    ...
    response = Response(response=json.dumps({"msg": "Fulfillment response"}),
                        status=200,
...
def test_violation():
    ...
    response = requests.get(url="http://shipping.default.svc.cluster.local:80/test_pii", headers=request_headers)
    # No response since it failed
    # response_labels.append(response.headers.get("X-Response-Labels"))
    # print("response_labels:", response_labels, type(response_labels))
