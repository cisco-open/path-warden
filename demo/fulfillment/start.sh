...
echo "${LGrn}Deploying Demo fulfillment...${NC}"
kubectl apply -f "$(dirname $0)/deploy.yml"
kubectl apply -f "$(dirname $0)/opal-client.yml"

#pod=$(echo $(kubectl get pods -l "app.kubernetes.io/name=fulfillment" -o custom-columns=:metadata.name | tr -d "\n"))
...
echo "${LGrn}Demo fulfillment successfully launched!"
