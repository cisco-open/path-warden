...
def test():
    ...
    response = Response(response=json.dumps({"msg": "Inventory response"}),
                        status=200,
...
@app.route("/test_tpsd", methods=['GET'])
@enable_tracing("GET_test_tpsd")
@save_trace_to_db(mysql=mysql)
@manage_request_labels(mysql=mysql)
@manage_response_labels(mysql=mysql)
def test_tpsd():
    ...
    response_labels = [os.getenv("service_label"), "tpsd"]
