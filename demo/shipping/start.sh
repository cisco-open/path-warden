...
echo "${LGrn}Deploying Demo Shipping...${NC}"
kubectl apply -f "$(dirname $0)/deploy.yml"
kubectl apply -f "$(dirname $0)/opal-client.yml"

#pod=$(echo $(kubectl get pods -l "app.kubernetes.io/name=shipping" -o custom-columns=:metadata.name | tr -d "\n"))
...
echo "${LGrn}Demo Shipping successfully launched!"
