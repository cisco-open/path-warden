...
def index():
    return "Hello Transaction-Processing!"
...
def test():
    ...
    response = requests.get(url="http://accounting.default.svc.cluster.local:80/test", headers=request_headers)
    response_labels.append(response.headers.get("X-Response-Labels"))
    print("response_labels:", response_labels, type(response_labels))

    response = requests.get(url="http://customer-data.default.svc.cluster.local:80/test", headers=request_headers)
    response_labels.append(response.headers.get("X-Response-Labels"))
    print("response_labels:", response_labels, type(response_labels))
    ...
    response = Response(response=json.dumps({"msg": "Transaction-processing response"}),
                        status=200,
