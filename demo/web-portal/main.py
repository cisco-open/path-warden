...
def test():
    ...
    response = requests.get(url="http://transaction-processing.default.svc.cluster.local:80/test", headers=request_headers)
    response_labels.append(response.headers.get("X-Response-Labels"))
    print("response_labels:", response_labels, type(response_labels))

    response = requests.get(url="http://fulfillment.default.svc.cluster.local:80/test", headers=request_headers)
    response_labels.append(response.headers.get("X-Response-Labels"))
    print("response_labels:", response_labels, type(response_labels))
    ...
    response = Response(response=json.dumps({"msg": "Web-portal response"}),
                        status=201,
...
@enable_tracing("GET_test_violation")
@save_trace_to_db(mysql=mysql)
@manage_request_labels(mysql=mysql)
@manage_response_labels(mysql=mysql)
def test_violation():
    ...
    response = requests.get(url="http://fulfillment.default.svc.cluster.local:80/test_violation", headers=request_headers)
    response_labels.append(response.headers.get("X-Response-Labels"))
    print("response_labels:", response_labels, type(response_labels))
